...
def encode_ingredients(recipes):
    ...
    recipes["ingredients_str"] = recipes["ingredients"].apply(
...
def sanity_check(recipes, ingredient_matrix, normalized_features):
    ...
    total_elements = ingredient_matrix.shape[0] * ingredient_matrix.shape[1]
    print(f"Non-zero values: {ingredient_matrix.nnz} out of {total_elements}")
    ...
    sns.heatmap(
        recipes[numerical_columns].corr(),
        annot=True,
        cmap="coolwarm",
        fmt=".2f",
        linewidths=0.5,
