...
def main():
    ...
    if scraping_choice == "y":
        ...
        if mode_choice == "1":
            ...
            asyncio.run(
                scraper.scrape_dataset(selected_features, batch_size=50, delay_between_batches=1.0)
            )
        elif mode_choice == "2":
            print("Starting conservative async scraping...")
            asyncio.run(
                scraper.scrape_dataset(selected_features, batch_size=20, delay_between_batches=2.0)
            )
        elif mode_choice == "3":
    ...
    try:
        ...
        enriched_features = enriched_features[enriched_features["success"]]
        print(
            f"Successfully merged data: {len(enriched_features)} recipes with enriched ingredients"
        )
    ...
    recommender = RecipeRecommender(enriched_features)  # k=5: Number of recommendations to make
    ...
    if "detailed_ingredients" in recommendations.columns:
        ...
        for _idx, recipe in recommendations.head(3).iterrows():
            print(f"\n--- {recipe['name']} ---")
