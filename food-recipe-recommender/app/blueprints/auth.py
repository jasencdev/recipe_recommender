...
from .. import db
from ..models import User, PasswordResetToken, PasswordResetRequestLog
...
def register() -> Tuple[Response, int]:
    ...
    try:
        ...
        logger.info("[register] successful registration | email=%s | user_id=%s", email, user.user_id)
        return jsonify({'success': True, 'message': 'Account created successfully'})
    except Exception as e:
        db.session.rollback()
        logger.error("[register] failed | email=%s | error=%s", email, str(e))
...
def auth_me() -> Tuple[Response, int]:
    if current_user.is_authenticated:
        return jsonify({'authenticated': True, 'user': {'id': current_user.user_id, 'email': current_user.email_address}})
...
def forgot_password():
    try:
        ...
        generic_response = jsonify({'success': True, 'message': 'If that email exists, a reset link has been sent.'})
        window_start = datetime.utcnow() - timedelta(hours=1)
        try:
            ip_count = PasswordResetRequestLog.query.filter(PasswordResetRequestLog.ip == ip, PasswordResetRequestLog.created_at >= window_start).count()
        except Exception:
            ip_count = 0
        ...
        if not user:
            ...
            return generic_response
        try:
            email_count = PasswordResetRequestLog.query.filter(PasswordResetRequestLog.email == email_norm, PasswordResetRequestLog.created_at >= window_start).count()
        except Exception:
            email_count = 0
