...
Delegate behaviors to single-source implementations to avoid drift.
"""
db = app_pkg.db
User = app_pkg.User
...
rate_limit_counts_impl = (
    app_pkg._rate_limit_counts_impl
    if hasattr(app_pkg, "_rate_limit_counts_impl")
    else app_pkg._rate_limit_counts
)  # type: ignore
...
def _require_admin() -> bool:
    # Delegate to package-level single-source implementation
    return app_pkg._require_admin()
...
def send_password_reset_email(to_email: str, token: str) -> bool:
    """Shim wrapper mirroring package-level legacy behavior.
    Respects module-level RESEND_* and resend patches in tests.
    """
    ...
    try:
        ...
        if RESEND_API_KEY and RESEND_AVAILABLE and resend is not None:
            ...
            # Fall back to package utils implementation
...
# Create default app instance only for test/CI legacy flows
import os as _os

if (_os.getenv("ENV", "development") or "").lower() in {"test", "ci"}:
