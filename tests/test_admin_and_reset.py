...
def test_admin_status_with_token(client, monkeypatch):
    ...
    r = client.get(
        ...
        query_string={"email": "x@example.com", "ip": "1.2.3.4"},
    )
    assert r.status_code == 200
...
def test_reset_password_flow(client):
    ...
    with client.application.app_context():
        ...
        token = PasswordResetToken(
            user_id=user.user_id, token="tok123", expires_at=datetime.utcnow() + timedelta(hours=1)
        )
    ...
    r = client.post(
        "/api/reset-password",
        data=json.dumps({"token": "tok123", "password": "newpass"}),
        content_type="application/json",
    )
    assert r.status_code == 200
    ...
    r = client.post(
        "/api/reset-password",
        data=json.dumps({"token": "tok123", "password": "again"}),
        content_type="application/json",
    )
    assert r.status_code == 400
