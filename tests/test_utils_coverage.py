...
from app import parse_list_field, send_password_reset_email, _require_admin as require_admin, _rate_limit_counts as rate_limit_counts  # type: ignore
from app import PasswordResetRequestLog  # type: ignore
...
class TestSendPasswordResetEmail:
    ...
    def test_send_password_reset_email_no_resend_key(self, client):
        """Test email sending when no RESEND_API_KEY is configured."""
    ...
    def test_send_password_reset_email_production_no_key(self, client):
        """Test email sending in production without key."""
        with client.application.app_context():
            client.application.config['RESEND_API_KEY'] = None
...
class TestRequireAdmin:
    ...
    def test_require_admin_correct_token(self, client):
        ...
        with client.application.test_request_context(headers={'X-Admin-Token': 'secret-admin-token'}):
            result = require_admin()
            assert result is True
