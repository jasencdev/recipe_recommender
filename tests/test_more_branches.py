...
def register_user(client, email="u@example.com"):
    r = client.post(
        "/api/register",
        data=json.dumps({"email": email, "password": "pw", "full_name": "U"}),
...
def test_save_recipe_found_by_food_recipe_id(client, monkeypatch):
    ...
    r = client.post(
        "/api/saved-recipes", data=json.dumps({"recipe_id": "500"}), content_type="application/json"
    )
    assert r.status_code == 200
...
def test_save_recipe_found_by_string_recipe_id(client, monkeypatch):
    ...
    r = client.post(
        "/api/saved-recipes",
        data=json.dumps({"recipe_id": "RID-123"}),
        content_type="application/json",
    )
    assert r.status_code == 200
...
def test_save_recipe_found_by_integer_index(client, monkeypatch):
    ...
    r = client.post(
        "/api/saved-recipes", data=json.dumps({"recipe_id": "77"}), content_type="application/json"
    )
    assert r.status_code == 200
...
def test_save_recipe_found_by_string_index(client, monkeypatch):
    ...
    r = client.post(
        "/api/saved-recipes",
        data=json.dumps({"recipe_id": "key1"}),
        content_type="application/json",
    )
    assert r.status_code == 200
...
def test_forgot_password_dev_token_path(client, monkeypatch):
    ...
    client.post(
        "/api/register",
        data=json.dumps({"email": "dev@example.com", "password": "pw", "full_name": "Dev"}),
        content_type="application/json",
    )
    ...
    monkeypatch.setattr(
        app_module, "send_password_reset_email", lambda *a, **k: False, raising=False
    )
...
def test_admin_cleanup_failure(client, monkeypatch):
    ...
    try:
        r = client.post(
            "/api/admin/maintenance/clear-reset-data",
...
def test_reset_password_commit_failure(client, monkeypatch):
    ...
    client.post(
        "/api/register",
        data=json.dumps({"email": "rp@example.com", "password": "pw", "full_name": "R"}),
        content_type="application/json",
    )
    from app import PasswordResetToken, User, db  # type: ignore
    ...
    with client.application.app_context():
        ...
        token = PasswordResetToken(
            ...
            + pd.Timedelta(hours=1).to_pytimedelta(),
        )
    ...
    try:
        r = client.post(
            "/api/reset-password",
            data=json.dumps({"token": "TK1", "password": "new"}),
            content_type="application/json",
        )
        assert r.status_code == 500
    finally:
