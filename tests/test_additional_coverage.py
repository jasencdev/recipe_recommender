...
def test_register_login_negative(client):
    ...
    r = client.post(
        "/api/login",
        data=json.dumps({"email": "dup@example.com", "password": "wrong"}),
        content_type="application/json",
    )
    assert r.status_code == 401
...
def test_forgot_password_paths(client, monkeypatch):
    ...
    r1 = client.post(
        "/api/forgot-password",
        data=json.dumps({"email": "lim@example.com"}),
        content_type="application/json",
        headers=headers,
    )
    r2 = client.post(
...
def test_admin_cleanup_success(client, monkeypatch):
    ...
    from app import PasswordResetRequestLog, PasswordResetToken, User, db  # type: ignore
    ...
    with client.application.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        old_log = PasswordResetRequestLog(
            email="e@example.com", ip="1.1.1.1", created_at=datetime.utcnow() - timedelta(days=10)
        )
        old_tok = PasswordResetToken(
...
def test_save_recipe_non_numeric_not_found(client, monkeypatch):
    ...
    r = client.post(
        "/api/register",
        data=json.dumps({"email": "savetest@example.com", "password": "pw", "full_name": "Save T"}),
        content_type="application/json",
    )
    assert r.status_code == 200
    ...
    r = client.post(
        "/api/saved-recipes", data=json.dumps({"recipe_id": "abc"}), content_type="application/json"
    )
    assert r.status_code == 400
